...
void GLViewNewModule::updateWorld()
{
   ...
   //if (NetMessengerClient::New("127.0.0.1", "2683")->isTCPSocketOpen()) {
	  // cout << "!!!!!!!!!!!!!!!!!!" << endl;
   //}
...
void GLViewNewModule::onMouseDown( const SDL_MouseButtonEvent& e )
{
   ...
   nmc->setCamPos(cam->getPosition());
   nmc->setCamLookDir(cam->getLookDirection());
   nmc->setObjPos(actorLst->at(0)->getPosition());
   nmc->setRotateZ(0.0f);

   this->client->sendNetMsgSynchronousTCP(*nmc);
}
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if( key.keysym.sym == SDLK_DOWN)
   {
	   ...
	   nmc->setObjPos(actor->getPosition());
	   nmc->setRotateZ(0.0f);
   }
